...
public class Dados {
    ...
    public static void deleteConfig(String indiceS) {

        int idConfig = new CanalConfigDAO().buscarConfigReturnId(indiceS);

        Connection con = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;
        try {

            stmt = con.prepareStatement("DELETE FROM tbl_Config WHERE id_Config = " + idConfig);
            stmt.execute();
            JOptionPane.showMessageDialog(null, "Config excluído com sucesso!");

        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Erro ao excluir config: " + ex);
        }
    ...
    public static void deleteUser(int idUser) {
        ...
        try {
            ...
            stmt = con.prepareStatement("DELETE FROM tbl_Usuario WHERE id_User = " + idUser + " limit 1");
            stmt.execute();
